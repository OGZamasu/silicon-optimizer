...
Optional:
  ...
  --page-url URL     Accepted for compatibility; staged page edits never
                     alter the wrapper until separately approved and applied.
...
The agent should insert variant HTML at insertLine.`);
  ...
  // The page-token stash is untrusted. A broad Generate approval must not
  // turn subsequently staged copy into an implicit source edit. The trusted
  // controller's digest-bound Apply is the only route that commits drafts.
