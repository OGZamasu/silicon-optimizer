...
struct RemoteArtifactSecurityTests {
    ...
    /// The URL screen and the socket veto must be one judgment. They were two copies, and
    /// the Swift one let through literals the C one then vetoed at connect — the wrong
    /// error, and no fallback to the provider's next URL — while refusing NAT64 literals
    /// the transfer would have taken.
    @Test func theURLScreenAndTheSocketVetoAgreeOnEveryAddressLiteral() {
        let policy = RemoteURLPolicy.publicHTTPS
        let base = URL(string: "https://provider.example.com")!
        let literals = [
            "127.0.0.1", "10.1.2.3", "172.16.0.1", "192.168.1.1", "169.254.169.254",
            "100.64.0.9", "0.0.0.0", "224.0.0.1", "192.0.0.8", "192.0.2.1", "198.18.0.1",
            "198.51.100.1", "203.0.113.5", "1.1.1.1", "8.8.8.8", "198.20.0.1",
            "::1", "fe80::1", "fd00::1", "2001:db8::1", "2606:4700:4700::1111",
            "2001:4860:4860::8888",
            // The ones that disagreed: IPv4-mapped, 6to4, Teredo, 3fff::/20 and NAT64.
            "::ffff:8.8.8.8", "::ffff:127.0.0.1", "2002:808:808::1", "2002:0a00:0001::1",
            "2001:0:4136:e378::1", "3fff::1", "64:ff9b::808:808", "64:ff9b::7f00:1",
            "64:ff9b::6440:9",
        ]
        for literal in literals {
            let host = literal.contains(":") ? "[\(literal)]" : literal
            let screened = policy.resolve("https://\(host)/a.mp3", relativeTo: base) != nil
            let connectable = literal.withCString(silicon_artifact_public_ip) == 1
            #expect(screened == connectable, "\(literal): URL \(screened), socket \(connectable)")
        }
        #expect(policy.resolve("https://[::ffff:8.8.8.8]/a.mp3", relativeTo: base) == nil)
        #expect(policy.resolve("https://[64:ff9b::808:808]/a.mp3", relativeTo: base) != nil)
    }
